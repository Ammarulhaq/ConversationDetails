...
import com.example.conversationdetails.adapters.ConversationListingAdapter
import com.example.conversationdetails.modelclasses.ConversationsItem
...
class ConversationFragment() : Fragment() ,ConversationListingAdapter.OnConversationSelect{
    ...
    {
        WebServicesHandler.instance.getAllConversations("eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJjb21wYW55X2lkIjoiMTg4IiwiZW1haWwiOiJoYW1tYWRAd3VycWkuY29tIiwicmVzb3VyY2VfaWQiOjEzMiwiZXhwIjoxNTkxMzQyODc4fQ.lJXWPQiS_vmjiUddsD_7TCeDenKX2XTAAywBugaIkrg",object : CustomCallback<RetrofitJSONResponse?>() {
            override fun onSuccess(response: RetrofitJSONResponse) {
                ...
                madapter =
                    ConversationListingAdapter(
                        context,
                        ListConvo,this@ConversationFragment
                    )
    ...
    override fun onSelectConversation(position: Int,conversationId:Int,msgId:Int) {
            notify?.onFragmentClicked(position,conversationId,msgId)
    }

  public interface notifyActivity
  {
      fun onFragmentClicked(position: Int,conversationId: Int,msgId: Int);
  }
}